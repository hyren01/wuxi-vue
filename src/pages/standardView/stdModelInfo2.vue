            ...
            <template v-slot:items="reData3">
              <td>
                <v-edit-dialog lazy>
                  ...
                  <template v-slot:input>
                    <v-text-field clearable :rules="[rules.code]" v-model="reData3.item.code"></v-text-field>
                  </template>
                </v-edit-dialog>
              </td>
              <td>
                <v-edit-dialog lazy>
                  {{ reData3.item.enName }}
                  <template v-slot:input>
                    <v-text-field clearable :rules="[rules.enName]" v-model="reData3.item.enName"></v-text-field>
                  </template>
                </v-edit-dialog>
              </td>
              <td>
                <v-edit-dialog lazy>
                  {{ reData3.item.name }}
                  <template v-slot:input>
                    <v-text-field clearable :rules="[rules.cnName]" v-model="reData3.item.name"></v-text-field>
                  </template>
                </v-edit-dialog>
              </td>
              <td>
              ...
              <td>
                <v-edit-dialog lazy>
                  {{ reData3.item.type }}
                  <template v-slot:input>
                    <v-text-field clearable v-model="reData3.item.type"></v-text-field>
                  </template>
                </v-edit-dialog>
              </td>
              <td>
                <v-edit-dialog lazy>
                  ...
                  <template v-slot:input>
                    <v-text-field clearable :rules="[rules.number]" v-model="reData3.item.maxsize"></v-text-field>
                  </template>
                </v-edit-dialog>
              </td>
              <td>
                <v-edit-dialog lazy>
                  {{ reData3.item.range }}
                  <template v-slot:input>
                    <v-text-field clearable :rules="[rules.number]" v-model="reData3.item.range"></v-text-field>
                  </template>
                </v-edit-dialog>
              </td>
              <td>
              ...
              <td>
                <v-icon small @click="deleteProperty2(reData3.index)">delete</v-icon>
              </td>
            </template>
          </v-data-table>
          <v-layout row wrap>
            ...
            <v-flex sm6 md6 xl6>
              <v-btn color="success" @click="handlecancel3" style="float:right">确定</v-btn>
            </v-flex>
          </v-layout>
        </div>
      </v-widget>
    ...
    <v-layout row wrap>
      <v-flex md12 class="align-center layout px-4 pt-4">
        ...
        <div class="page-header-right">
          <v-btn color="success" @click="finish($route.query.id, modelInfo)">保存</v-btn>
          <v-btn icon>
          ...
          <div slot="widget-content">
            <div class="layout">
              ...
              <div class="grey--text mr-3 my-2 text--darken-2">
                <v-edit-dialog lazy>
                  {{ modelInfo.code }}
                  <template v-slot:input>
                    <v-text-field clearable :rules="[rules.enName]" v-model="modelInfo.code"></v-text-field>
                  </template>
                </v-edit-dialog>
              </div>
            </div>
            ...
            <div class="layout">
              ...
              <div class="grey--text mr-3 my-2 text--darken-2">
                <v-edit-dialog lazy>
                  {{ modelInfo.enName }}
                  <template v-slot:input>
                    <v-text-field clearable :rules="[rules.enName]" v-model="modelInfo.enName"></v-text-field>
                  </template>
                </v-edit-dialog>
              </div>
            </div>
            ...
            <div class="layout">
              ...
              <div class="grey--text mr-3 my-2 text--darken-2">
                <v-edit-dialog lazy>
                  {{ modelInfo.name }}
                  <template v-slot:input>
                    <v-text-field clearable :rules="[rules.cnName]" v-model="modelInfo.name"></v-text-field>
                  </template>
                </v-edit-dialog>
              </div>
            </div>
            ...
            <div class="layout">
              ...
              <div class="grey--text mr-3 my-2 text--darken-2">
                ...
                <!-- <template v-slot:input>
                    <v-text-field clearable v-model="modelInfo.categoryId"></v-text-field>
                  </template>
                </v-edit-dialog>-->
              </div>
            </div>
            ...
            <div class="layout">
              ...
              <div class="grey--text mr-3 my-2 text--darken-2">
                ...
                <!-- <template v-slot:input>
                    <v-text-field clearable v-model="modelInfo.createPerson"></v-text-field>
                  </template>
                </v-edit-dialog>-->
              </div>
            </div>
            ...
            <div class="layout">
              ...
              <v-spacer></v-spacer>
              <div class="grey--text mr-3 my-2 text--darken-2">{{ modelInfo.createTime }}</div>
            </div>
            <v-divider></v-divider>
            <div class="layout">
              ...
              <div class="grey--text mr-3 my-2 text--darken-2">
                ...
                <!-- <template v-slot:input>
                    <v-text-field clearable v-model="modelInfo.updatePerson"></v-text-field>
                  </template>
                </v-edit-dialog>-->
              </div>
            </div>
            ...
            <div class="layout">
              ...
              <v-spacer></v-spacer>
              <div class="grey--text mr-3 my-2 text--darken-2">{{ modelInfo.updateTime }}</div>
            </div>
            <v-divider></v-divider>
            <div class="layout">
              <div class="ml-3 my-2">版本</div>
              <v-spacer></v-spacer>
              <div class="grey--text mr-3 my-2 text--darken-2">{{ modelInfo.version }}</div>
            </div>
          </div>
        </v-widget>
      </v-flex>
      <v-flex sm12 md8 xl9>
        <v-alert color="white" icon="info" :value="true" class="ma-0 elevation-1 black--text">
          <div>
              ...
              <template v-slot:items="reData2">
                <td>
                  <v-edit-dialog lazy>
                    ...
                    <template v-slot:input>
                      <v-text-field clearable v-model="reData2.item.objCname"></v-text-field>
                    </template>
                  </v-edit-dialog>
                </td>
                <td>
                  <v-edit-dialog lazy>
                    ...
                    <template v-slot:input>
                      <v-text-field clearable v-model="reData2.item.objEname"></v-text-field>
                    </template>
                  </v-edit-dialog>
                </td>
                <td>
                  <v-edit-dialog lazy>
                    ...
                    <template v-slot:input>
                      <v-text-field clearable v-model="reData2.item.shortName"></v-text-field>
                    </template>
                  </v-edit-dialog>
                </td>
                <td>
                  <v-edit-dialog lazy>
                    ...
                    <template v-slot:input>
                      <v-text-field clearable v-model="reData2.item.objDefined"></v-text-field>
                    </template>
                  </v-edit-dialog>
                </td>
                <td>
              ...
              <template v-slot:items="props">
                ...
                <td class="px-1">
                  <v-edit-dialog lazy>
                    {{ props.item.name }}
                    <template v-slot:input>
                      <v-text-field clearable v-model="props.item.name"></v-text-field>
                    </template>
                  </v-edit-dialog>
                </td>
                <td class="px-1">
                ...
                <td class="px-1">
                  <v-edit-dialog lazy>
                    ...
                    <template v-slot:input>
                      <v-text-field clearable v-model="props.item.maxContains"></v-text-field>
                    </template>
                  </v-edit-dialog>
                </td>
                <td class="px-1">
...
export default {
  ...
  data: () => ({
    ...
    rules: {
      ...
      number: value => /^[0-9]+$/.test(value) || "请填写数字",
      enName: value =>
  ...
  methods: {
    ...
    // 修改方法
    finish() {
      if (!this.testnull(this.modelInfo.code)) {
        this.showSnackbar("短名不能为空，请填写", "error");
        return;
      }
      if (!this.testnull(this.modelInfo.enName)) {
        this.showSnackbar("英文名不能为空，请填写", "error");
        return;
      }
      if (!this.testnull(this.modelInfo.name)) {
        this.showSnackbar("中文名不能为空，请填写", "error");
        return;
      }
      if (!this.testEnglish(this.modelInfo.code)) {
        this.showSnackbar("短名请输入英文，请填写", "error");
        return;
      }
      if (!this.testEnglish(this.modelInfo.enName)) {
        this.showSnackbar("英文名请输入英文，请填写", "error");
        return;
      }
      if (!this.testChinese(this.modelInfo.name)) {
        this.showSnackbar("中文名请输入中文，请填写", "error");
        return;
      }
      ...
      for (let a of this.modelInfo.standardField) {
        ...
        if (!this.testnull(a.maxsize)) {
          this.showSnackbar("字段长度不能为空，请填写", "error");
          return;
        }
        if (!this.testnull(a.maxContains)) {
          this.showSnackbar("字段最大出现次数不能为空，请填写", "error");
          return;
        }
        if (!this.testnull(a.range)) {
          this.showSnackbar("字段值域不能为空，请填写", "error");
          return;
        }

        if (!this.testEnglish(a.enName)) {
        ...
        if (!this.testnumber(a.maxContains)) {
          this.showSnackbar("字段最大出现次数请输入数字", "error");
          return;
        }
        if (!this.testnumber(a.range)) {
          this.showSnackbar("字段范围请输入数字", "error");
          return;
        }
